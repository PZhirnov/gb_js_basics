...
// Получим элементы из формы
let btns = document.querySelectorAll('button');
let prodDiv = document.querySelector('.products');

// Назначим событие на клик кнопки
btns.forEach((btn) => {
 ...
 */
 function showCategory(category) {
    ...
    let showMark = "";
    catProd.forEach((prod) => {
        showMark += getProductMarkup(prod);
    });
    // выводим в разметку
    prodDiv.insertAdjacentHTML("afterbegin", showMark);
}
...
function getProductMarkup(product) {
    ...
    let resShow = `
                ...
                <img src="${imgProd}" alt="">
                <div>${priceProd}</div>
                <a href="https://example.com/producs/${idProd}">Подробнее</a>
                </div>`;
    return resShow;
}

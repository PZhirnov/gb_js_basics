...
function hideMoreText(card) {
    ...
    let elDesc = card.wrap.querySelector('.desc');
    elDesc.remove();
...
function showMoreText(card) {
    ...
    const txtProd = 'Тут текст, который надо вставить вместо картинки'
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    let txtDiv = `<div class = 'desc'>${txtProd}</div>`;
    card.productName.insertAdjacentHTML('afterend', txtDiv); // вставим текст в div
